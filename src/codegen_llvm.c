...
typedef struct {
    const DiAstProgram *program;
} Ctx;

static const DiAstDecl *find_struct_decl(const DiAstProgram *program, const char *type_name) {
    size_t i;
    for (i = 0; i < program->decl_count; ++i) {
        if (program->decls[i]->kind == DI_AST_STRUCT_DECL &&
...
static const char *decl_symbol_name(const DiAstDecl *decl) {
    static char buffer[16][128];
    static int index = 0;

    if (decl->owner_type == NULL) {
        return map_runtime_symbol(decl->name);
    }
    index = (index + 1) % 16;
    snprintf(buffer[index], sizeof(buffer[index]), "%s_%s", decl->owner_type, decl->name);
    return buffer[index];
}

static const char *c_type_name(const DiAstProgram *program, const char *type_name) {
    static char buffer[16][128];
    static int index = 0;
...
static void emit_stmt_c(FILE *out, const DiAstProgram *program, const DiAstStmt *stmt, int indent);

static void emit_stmt_list_c(FILE *out, const DiAstProgram *program, DiAstStmt **items, size_t count, int indent) {
    size_t i;
    for (i = 0; i < count; ++i) {
...
static void emit_flux_stmt_c(FILE *out, const DiAstProgram *program, const DiAstStmt *stmt, int indent) {
...
static int decl_has_array_stmt(const DiAstDecl *decl) {
    size_t i;
...
int di_codegen_emit_llvm_ir(const DiAstProgram *program, const char *input_path) {
    ...
    for (i = 0; i < program->decl_count; ++i) {
        ...
        if (decl->kind == DI_AST_FUNCTION || decl->kind == DI_AST_EXTERN_FUNCTION) {
            fprintf(out, "define %s @%s(...) {\n", strcmp(decl->return_type.name, "void") == 0 ? "void" : "i32", decl_symbol_name(decl));
...
int di_codegen_build_native(const DiAstProgram *program, const char *input_path, int run_after_build) {
    ...
    for (i = 0; i < program->decl_count; ++i) {
        const DiAstDecl *decl = program->decls[i];
        size_t j;
        if (decl->kind != DI_AST_STRUCT_DECL) continue;
        fprintf(out, "struct %s {\n", decl->name);
        for (j = 0; j < decl->field_count; ++j) {
            fprintf(out, "    %s %s;\n", c_type_name(program, decl->fields[j].type.name), decl->fields[j].name);
        }
        fprintf(out, "};\n\n");
    }
    ...
    for (i = 0; i < program->decl_count; ++i) {
        ...
        if (decl->kind == DI_AST_STRUCT_DECL) continue;
        fprintf(out, "%s %s(",
                c_type_name(program, decl->return_type.name),
                decl_symbol_name(decl));
        for (j = 0; j < decl->param_count; ++j) {
            if (j != 0) fprintf(out, ", ");
            fprintf(out, "%s %s",
                    c_type_name(program, decl->params[j].type.name),
                    decl->params[j].name);
        }
