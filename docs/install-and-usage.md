...
- `diva new --kernel` is **seed-only** (use `bootstrap/diva-linux-amd64`). **`diva build` / `diva run`** use pure ELF by default; set **`DIVA_ALLOW_HOSTED_LINK=1`** to use **`cc` + `DI_RUNTIME_O`** when you need the hosted runtime link path.
