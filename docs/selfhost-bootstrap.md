...
This document tracks what the hosted Diva runtime exposes so a Diva-implemented compiler can exist without growing the language surface first.
...
`str` values are nul-terminated byte strings in the host. For lexer-style access without a `byte` type in Diva:
...
Do not declare a top-level function named `di_user_main` in Diva sources; it is reserved for codegen.
