...
- **Pipeline (`ir`, `asm`, `run`, …):** the pure-built driver can **SIGSEGV** early (e.g. `ir` on a tiny file). A ptrace probe shows **`rip == 0`** and **`[[rsp]] == 0`** — consistent with **control flow to a null address** (not the earlier “small handle vs mmap pointer” fault in `str_builder_*` stubs alone).
