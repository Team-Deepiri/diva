...
# Exception: compiler/res/legacy/ may ship hosted cc-link helpers (see README.txt there).
set -eu
ROOT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
cd "${ROOT_DIR}"

bad=$(git ls-files | grep -E '\.(c|h|cc|cxx|cpp|hpp|hh)$' | grep -v '^compiler/res/legacy/' || true)
if [ -n "${bad}" ]; then
